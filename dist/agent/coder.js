...
exports.executeCoderTask = executeCoderTask;
...
async function getRepoTree(repo, path = "") {
    try {
        const url = path
        ...
        if (!Array.isArray(items))
            return [];
        return items.map((i) => `${i.type === "dir" ? "📁" : "📄"} ${i.path}`);
    }
    catch (e) {
...
async function createBranch(repo, branchName, fromBranch = "main") {
    try {
        ...
        if (!sha)
            return { success: false, error: `branch ${fromBranch} não encontrada` };
...
async function executeCoderTask(task, repo, context = "") {
    const results = [];
    let iteration = 0;
    ...
    while (iteration < maxIterations) {
        ...
        const response = await (0, tools_1.generateResponse)(messages, CODER_SYSTEM_PROMPT);
        ...
        for (const action of actions) {
            try {
                switch (action.action) {
                    ...
                    case "create_file":
                    case "update_file": {
                        if (!action.repo || !action.path || !action.content)
                            break;
                        const res = await createOrUpdateFile(action.repo, action.path, action.content, action.message || `Opencrawsbuties: ${action.action} ${action.path}`);
                        if (res.success) {
                    ...
                    case "delete_file": {
                        if (!action.repo || !action.path)
                            break;
                        const res = await deleteFile(action.repo, action.path, action.message || `Opencrawsbuties: deletar ${action.path}`);
                        results.push(res.success ? `🗑️ Deletado: ${action.path}` : `❌ Erro: ${res.error}`);
                        break;
                    }
                    case "create_branch": {
                        if (!action.repo || !action.branch)
                            break;
                    ...
                    case "create_pr": {
                        if (!action.repo || !action.title)
                            break;
                        const res = await createPR(action.repo, action.title, action.body || "", action.head || "main", action.base || "main");
