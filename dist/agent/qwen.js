...
exports.askQwenCoder = askQwenCoder;
...
async function askQwenCoder(prompt, context) {
    ...
    if (context) {
        messages.push({ role: 'user', content: `Contexto do codigo:\n${context}` });
    }
    ...
    try {
        const response = await groq.chat.completions.create({
            ...
            messages: [
                ...
                ...(context ? [{ role: 'user', content: `Contexto do codigo:\n${context}` }] : []),
