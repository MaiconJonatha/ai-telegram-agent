...
exports.generateFlowImage = generateFlowImage;
exports.generateFlowVideo = generateFlowVideo;
...
async function clickCreate(page) {
    ...
    for (const btn of allButtons) {
        ...
        if (info.text.includes("arrow_forward") ||
...
async function downloadImageFromPage(page, imgElement) {
...
async function checkFailed(page) {
...
async function _generateFlowImageOnce(prompt) {
    let page = null;
    try {
        ...
        return { hasProgress, hasFail };
            ...
            if (progress.hasProgress) {
                if (i % 5 === 0)
                    console.log(`[${new Date().toISOString()}] [Flow/IMG] Progresso detectado, aguardando...`);
            ...
            if (i % 5 === 0)
                console.log(`[${new Date().toISOString()}] [Flow/IMG] Aguardando... ${i * 3}s`);
        }
...
async function generateFlowImage(prompt) {
...
async function _generateFlowVideoOnce(prompt) {
    let page = null;
    try {
        ...
        return { hasProgress, hasFail };
            ...
            if (progress.hasProgress) {
                if (i % 10 === 0)
                    console.log(`[${new Date().toISOString()}] [Flow/VID] Progresso detectado, aguardando...`);
                continue;
            }
            ...
            if (i % 10 === 0)
                console.log(`[${new Date().toISOString()}] [Flow/VID] Aguardando... ${i * 3}s`);
        }
        await page.close();
        return null;
    }
...
async function generateFlowVideo(prompt) {
