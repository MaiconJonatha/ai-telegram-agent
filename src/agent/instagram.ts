...
async function clickByText(page: Page, texts: string[], tag = "button"): Promise<boolean> {
...
async function clickByAriaLabel(page: Page, labels: string[]): Promise<boolean> {
...
export async function postToInstagram(
  ...
  try {
    ...
    for (const el of allElements) {
      const info = await page.evaluate((e: any) => {
    ...
    async function clickTextElement(pg: Page, texts: string[]): Promise<boolean> {
      return pg.evaluate(`((searchTexts) => {
        ...
        return false;
      })(${JSON.stringify(texts)})`) as Promise<boolean>;
    }

    // Helper: click button by aria-label
    async function clickAriaButton(pg: Page, labels: string[]): Promise<boolean> {
      for (const label of labels) {
        const btn = await pg.$(`button[aria-label="${label}"]`);
        if (btn) {
          const visible = await pg.evaluate((el: any) => {
            const rect = el.getBoundingClientRect();
    ...
    async function clickDialogHeaderButton(pg: Page, texts: string[]): Promise<boolean> {
      ...
      const coords = await pg.evaluate(`((searchTexts) => {
        ...
        return null;
      })(${JSON.stringify(texts)})`) as { x: number; y: number } | null;
    ...
    async function closePopupOK(pg: Page): Promise<boolean> {
      return pg.evaluate(`(() => {
        const btns = document.querySelectorAll("button");
        for (let i = 0; i < btns.length; i++) {
    ...
    for (let attempt = 0; attempt < 6 && !captionAdded; attempt++) {
      ...
      if (counts.ce === 0 && counts.tb === 0 && counts.ta === 0) {
        if (attempt < 5) continue; // Wait more
      }
      ...
      for (const sel of captionSelectors) {
        ...
        for (const field of fields) {
          ...
          if (size.w < 50 || size.h < 10) continue;
...
async function closeInstagramPopups(page: Page): Promise<void> {
