...
export async function createOrUpdateFile(
  ...
  try {
    ...
    const body: any = {
      ...
      committer: { name: "ArcanjoBot", email: "arcanjobot@users.noreply.github.com" },
    };
    if (sha) body.sha = sha;
        ...
        switch (action.action) {
          ...
          case "update_file": {
            ...
            const res = await createOrUpdateFile(
              ...
              action.path,
              action.content,
              action.message || `ArcanjoBot: ${action.action} ${action.path}`
            );
            if (res.success) {
          ...
          case "delete_file": {
            ...
            const res = await deleteFile(
              action.repo,
              action.path,
              action.message || `ArcanjoBot: deletar ${action.path}`
            );
